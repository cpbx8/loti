...
const GI_SYSTEM_PROMPT_ES = `You are a glycemic index expert specializing in Mexican and Latin American foods.
...
- swap_suggestion: If the food would result in high glycemic load (GL >= 11), suggest a culturally appropriate lower-GI Mexican alternative. Examples:
  - "Prueba tortilla de maiz en vez de harina - 30% menos impacto glucemico"
  - "Agrega nopales para reducir el impacto en tu glucosa"
  - "Prueba agua de jamaica sin azucar en vez de refresco"
...
const GI_SYSTEM_PROMPT_EN = `You are a glycemic index expert specializing in Mexican and Latin American foods.
  ...
  try {
    ...
    const res = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        temperature: 0.1,
        messages: [
          { role: "system", content: language === 'en' ? GI_SYSTEM_PROMPT_EN : GI_SYSTEM_PROMPT_ES },
